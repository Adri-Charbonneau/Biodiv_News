...
$name = "Martinia"
$accounts = ""
$tags ="#martinia #biodiversité #libellules #odonatologie #entomologie"

# compare two title
[xml]$old_title = Get-Content ./$id/$id.xml -Encoding UTF8
$old = $old_title.rss.channel.item.title[0]

[xml]$new_title = (invoke-webrequest -Uri "https://martinia.insectes.org/feed/").Content
$new = $new_title.rss.channel.item.title[0]

if ( $new -eq $old ) {
echo "Le dernier article de $name est déjà existant dans la base de donnée"
} else {
Invoke-WebRequest -Uri "https://martinia.insectes.org/feed/" -OutFile "./$id/$id.xml"
